 ...
 * Handles Excel exports for events, workforce, and other data
 */
...
class ExportManager {
    ...
    checkXLSXAvailability() {
        this.isXLSXLoaded = typeof XLSX !== 'undefined';
        if (!this.isXLSXLoaded) {
            console.warn('XLSX library not loaded. Export functionality will be limited.');
        }
    }
     ...
     */
    exportToExcelDetailed(events) {
        if (!this.isXLSXLoaded) {
            alert('Export functionality is not available. Please check if XLSX library is loaded.');
            return;
        }
     ...
     */
    exportToExcelSimplified(events) {
        if (!this.isXLSXLoaded) {
            alert('Export functionality is not available. Please check if XLSX library is loaded.');
            return;
        }
    ...
    /**
     * Export helpers details to Excel - Workforce focused export
     */
    exportHelpersToExcel(events) {
        if (!this.isXLSXLoaded) {
            alert('Export functionality is not available. Please check if XLSX library is loaded.');
            return;
        }
...
// Export singleton instance and make functions globally available
const exportManager = new ExportManager();

// Make functions globally available for backward compatibility
window.exportToExcelDetailed = (events) => exportManager.exportToExcelDetailed(events);
window.exportToExcelSimplified = (events) => exportManager.exportToExcelSimplified(events);
window.exportHelpersToExcel = (events) => exportManager.exportHelpersToExcel(events);

window.ExportManager = exportManager; 

function getSelectedVehicles() {
    // Încearcă să ia din DOM (pentru pasul de vehicule)
    const selects = document.querySelectorAll('.plate-select');
    if (selects.length > 0) {
    ...
    // Dacă nu există în DOM, ia din localStorage (pentru pasul de rută)
    const fromStorage = localStorage.getItem('selectedVehiclesForRoute');
    if (fromStorage) {
